...
import { PlusOutlined, EyeOutlined, DeleteOutlined, EditOutlined, CalculatorOutlined, PrinterOutlined } from '@ant-design/icons';
...
const formatAddress = (addr: {
    ...
    if (!addr) return '—';
    return [addr.unitNumber, addr.buildingName, addr.streetOrLandmark, addr.destination, addr.city, addr.state, addr.pincode]
        .filter(Boolean).join(', ');
};
...
const SalesBills = () => {
    ...
    const [printingBill, setPrintingBill] = useState<SalesBillResponse | null>(null);
    ...
    const handleAdd = () => {
        ...
        form.setFieldsValue({
            billDate: dayjs(),
            items: [{ itemId: undefined, quantity: undefined, rate: undefined }],
        });
        setIsModalOpen(true);
    };
    ...
    const handleEdit = (record: SalesBillResponse) => {
        ...
        form.setFieldsValue({
            ...
            buyerOrderNo: record.buyerOrderNo === 'VERBAL' ? '' : record.buyerOrderNo,
            paymentDue: record.paymentDue || (record as any).modeOfPayment,
    ...
    const handlePrint = (record: SalesBillResponse) => {
        setPrintingBill(record);
    };
    ...
    const handleOk = async () => {
        try {
            ...
            if (partyAddresses.length === 1) {
                billingAddressId = partyAddresses[0].id;
                shippingAddressId = partyAddresses[0].id;
            } else if (partyAddresses.length > 1) {
                if (addrMode === 'same') {
            ...
            const billData: SalesBillRequest = {
                ...
                buyerOrderNo: values.buyerOrderNo || undefined,   // omit = backend uses 'VERBAL'
                paymentDue: values.paymentDue || undefined, // omit = backend uses 'IMMEDIATE'
                modeOfPayment: values.paymentDue || undefined, // For backend compatibility
                termsOfDelivery: values.termsOfDelivery || undefined,
    ...
    const AddressSelect = ({ name, label }: { name: string; label: string }) => (
        <Form.Item name={name} label={label}>
            <Select placeholder="Select address (optional)" allowClear popupMatchSelectWidth={false}>
        ...
        <div>
            ...
            >
                ...
                >
                    ...
                    {/* Address selection */}
                    {selectedPartyId && partyAddresses.length > 1 && (
                        <div style={{
                    ...
                    <Space style={{ width: '100%' }} size="large">
                        <Form.Item name="buyerOrderNo" label="Buyer Order No." style={{ flex: 1 }}>
                            <Input placeholder="Leave blank for VERBAL order" />
                        </Form.Item>
                        <Form.Item name="paymentDue" label="Payment Due" style={{ flex: 1 }}>
                            <Select placeholder="Immediate (default)" allowClear popupMatchSelectWidth={false}>
                                {Object.entries(PAYMENT_DUE_LABELS).map(([value, label]) => (
                    ...
                    <Space style={{ width: '100%' }} size="large">
                        ...
                        <Form.Item name="termsOfDelivery" label="Terms of Delivery (Optional)" style={{ flex: 1 }}>
                            <Input placeholder="e.g. Ex-Works, FOR Destination" />
                        </Form.Item>
                    </Space>
                    <Form.Item name="transportation" label="Transportation (₹)" style={{ maxWidth: 240 }}>
                        <InputNumber
                ...
                extra={
                    viewingBill && (
                        <Button
                            ...
                            onClick={() => { setPrintingBill(viewingBill); }}
                        >
                            Print Invoice
                        </Button>
                    )
                ...
                {viewingBill && (
                    <div>
                        <div style={{ marginBottom: 24 }}>
                            ...
                            <p><strong>Buyer Order No.:</strong> {viewingBill.buyerOrderNo}</p>
                            <p><strong>Payment Due:</strong> {PAYMENT_DUE_LABELS[viewingBill.paymentDue || (viewingBill as any).modeOfPayment] ?? (viewingBill.paymentDue || (viewingBill as any).modeOfPayment)}</p>
                            {viewingBill.termsOfDelivery && (
                                <p><strong>Terms of Delivery:</strong> {viewingBill.termsOfDelivery}</p>
                            )}
                        ...
                        <Table
                            ...
                            columns={[
                                { title: 'Sr', dataIndex: 'serialNumber', key: 'serialNumber', width: 40 },
                                { title: 'Description', dataIndex: 'itemDescription', key: 'itemDescription' },
                                { title: 'HSN', key: 'hsn', width: 80, render: (_: any, row: any) => items.find(i => i.id === row.itemId)?.hsn ?? '—' },
            ...
            {printingBill && (
                <SalesInvoicePDF
                    ...
                    enrichedItems={printingBill.items.map(item => ({
                        ...item,
                        hsn: items.find(i => i.id === item.itemId)?.hsn,
                        unit: items.find(i => i.id === item.itemId)?.unit,
                    }))}
                />
            )}
        </div>
