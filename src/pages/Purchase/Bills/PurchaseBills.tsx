...
const PurchaseBills = () => {
    ...
    const handleAdd = () => {
        ...
        form.setFieldsValue({
            billDate: dayjs(),
            items: [{ description: '', quantity: null, rate: null }],
        });
        setIsModalOpen(true);
    };
                                ...
                                {fields.map((field, index) => (
                                    <Space key={field.key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                                        ...
                                        <Form.Item {...field} name={[field.name, 'quantity']}
                                            ...
                                            <InputNumber min={0.01} placeholder="Qty" style={{ width: 90 }} controls={false} onKeyPress={e => { if (!/[0-9.]/.test(e.key)) e.preventDefault(); }} />
                                        </Form.Item>
                                        <Form.Item {...field} name={[field.name, 'rate']}
                                            label={index === 0 ? 'Rate (₹)' : ''} rules={validateRate()}>
                                            <InputNumber min={0} placeholder="Rate" style={{ width: 130 }} prefix="₹" controls={false} onKeyPress={e => { if (!/[0-9.]/.test(e.key)) e.preventDefault(); }} />
                                        </Form.Item>
                                        {fields.length > 1 && (
