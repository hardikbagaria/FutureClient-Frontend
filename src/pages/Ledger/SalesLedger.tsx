...
import { getSalesPartyLedger, getSalesLedgerSummary, getSalesAllTransactions } from '@/api/ledger';
...
const SalesLedger = () => {
    ...
    const { data: summary = [], isLoading: summaryLoading, error: summaryError } = useQuery({
        queryKey: ['ledger', 'sales', 'all', activeStartDate, activeEndDate],
        queryFn: () => getSalesLedgerSummary({ startDate: activeStartDate, endDate: activeEndDate }),
    ...
    const { data: monthlyTransactions = [], isLoading: transactionsLoading, error: transactionsError } = useQuery({
        queryKey: ['ledger', 'sales', 'transactions', activeStartDate, activeEndDate, includePayments],
        queryFn: () => getSalesAllTransactions({ startDate: activeStartDate, endDate: activeEndDate, includePayments }),
    ...
    const { data: partyLedger, isLoading: ledgerLoading, error: ledgerError } = useQuery({
        queryKey: ['ledger', 'sales', 'party', selectedPartyId, activeStartDate, activeEndDate],
        queryFn: () => getSalesPartyLedger(selectedPartyId!, { startDate: activeStartDate, endDate: activeEndDate }),
    ...
    const handleExportPdf = () => {
        if (viewMode === 'summary') {
            exportSummaryToPdf('Sales', summary, dateLabel);
        } else if (viewMode === 'monthly_transactions') {
            exportMonthlyToPdf('Sales', monthlyTransactions, dateLabel);
        ...
        <div>
            ...
            {viewMode === 'summary' ? (
                ...
                transactionsError ? (
                    ...
                    <Card title="Sales Monthly Register">
