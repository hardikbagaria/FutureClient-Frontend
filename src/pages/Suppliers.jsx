...
import { Plus, Pencil, Trash2, MapPin, Phone, Building2, User, MapPinned } from 'lucide-react';
        ...
        <div className="animate-fade-in">
            <div className="page-header">
                <div>
                    ...
                    <p className="page-subtitle">Manage your purchase parties and vendors</p>
                </div>
                            ...
                            {party.addresses?.[0] && (
                                <div style={{ padding: '1rem 1.25rem', background: 'var(--color-bg-surface-hover)', fontSize: '0.875rem' }}>
                                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', color: 'var(--color-text-muted)' }}>
                                        ...
                                        <span>
                                            {party.addresses[0].building}, {party.addresses[0].street}, {party.addresses[0].city} - {party.addresses[0].pincode}
                                        </span>
                                    </div>
                                </div>
                            )}
            ...
            >
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={{ marginBottom: '2rem' }}>
                        <h4 style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--color-text-main)', marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                            Supplier Details
                        </h4>
                                ...
                                <div key={idx} style={{
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                            <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: 'var(--color-primary)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 600 }}>
                                                {idx + 1}
                                            </div>
                                            <select
                                                className="form-input"
                                        ...
                                        {formData.addresses.length > 1 && (
                                            <button type="button" className="btn btn-danger btn-sm btn-icon" onClick={() => removeAddress(idx)}>
                                                <Trash2 size={12} />
                                            </button>
                                        )}
                                    </div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '0.75rem' }}>
                                        <div className="form-group" style={{ marginBottom: 0 }}>
                                            ...
                                            <input type="text" className="form-input" value={addr.building} onChange={(e) => handleAddressChange(idx, 'building', e.target.value)} placeholder="Building" required />
                                        </div>
                                        <div className="form-group" style={{ marginBottom: 0 }}>
                                            <label className="form-label">Street *</label>
                                            <input type="text" className="form-input" value={addr.street} onChange={(e) => handleAddressChange(idx, 'street', e.target.value)} placeholder="Street" required />
                                        </div>
                                        <div className="form-group" style={{ marginBottom: 0 }}>
                                            <label className="form-label">Area *</label>
                                            <input type="text" className="form-input" value={addr.area} onChange={(e) => handleAddressChange(idx, 'area', e.target.value)} placeholder="Area" required />
                                        </div>
                                        <div className="form-group" style={{ marginBottom: 0 }}>
                                            <label className="form-label">City *</label>
                                            <input type="text" className="form-input" value={addr.city} onChange={(e) => handleAddressChange(idx, 'city', e.target.value)} placeholder="City" required />
                                        </div>
                                        <div className="form-group" style={{ marginBottom: 0 }}>
                                            <label className="form-label">Pincode *</label>
                                            <input type="text" className="form-input" value={addr.pincode} onChange={(e) => handleAddressChange(idx, 'pincode', e.target.value)} placeholder="Pincode" pattern="[0-9]{6}" required />
                                        </div>
                                        <div className="form-group" style={{ marginBottom: 0 }}>
                                            <label className="form-label">State *</label>
                                            <select className="form-input" value={addr.state} onChange={(e) => handleAddressChange(idx, 'state', e.target.value)} required>
