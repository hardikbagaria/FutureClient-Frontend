...
// Payment due used on Sales Bills (different set from payment receipts)
...
// Sales Address Request (for add / edit address endpoints)
export interface SalesAddressRequest {
    unitNumber: string;
    buildingName: string;
    streetOrLandmark?: string | null;
    destination: string;
...
// Used only for initial party creation — allows bundling addresses
export interface SalesPartyCreateRequest extends SalesPartyRequest {
    addresses: SalesAddressRequest[];
}
...
export interface SalesBillRequest {
    ...
    buyerOrderNo?: string;       // defaults to "VERBAL" on backend if omitted
    paymentDue?: PaymentDue; // defaults to "IMMEDIATE" on backend if omitted
    modeOfPayment?: PaymentDue | string; // backward compat API Support
...
export interface SalesBillResponse {
    ...
    buyerOrderNo: string;          // "VERBAL" when no written PO
    paymentDue: PaymentDue; // always present in response
    modeOfPayment?: string; // backward compat API Support
...
// ── Previous Balance DTOs ────────────────────────────────────────────────────

export interface PreviousBalanceRequest {
    partyId: number;
    amount: number;
}
